...
class Plugs extends Controller
{
    ...
    {
         return 'doc,gif,icon,jpg,mp3,png,xls,xlsx';
    }
    ...
    public function check()
    {
        ...
        $diff2 = explode(',', strtolower($this->extension()));
    ...
    {
        // return json(['uploaded' => false, 'error' => ['message' => '接口关闭']]);
        ...
        if (!$file->checkExt(strtolower($this->extension()))) {
        ...
        if (is_array($info) && isset($info['url'])) {
             return json(['uploaded' => true, 'filename' => $name, 'url' => '/upload/'.$name]);
    ...
    private function getUploadData()
    {
        if ($this->getUploadType() === 'qiniu') {
            ...
            return ['url' => '?s=' . url('api.plugs/upload'), 'token' => uniqid('local_upload_'), 'uptype' => $this->getUploadType()];
        }
    }
